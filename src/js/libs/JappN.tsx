...
export default class JappN{
    ...
    protected Cargar(vista, cont_callback?, callback_cont?){
        let cont, fnd
        if (typeof cont_callback=="string") cont=document.querySelector(cont_callback)
        if (typeof callback_cont=="string") cont=document.querySelector(callback_cont)
    ...
    private eliminarRecursivamente(item, exito):boolean {
        let grpExito:boolean=true
        let miExito:boolean=true
    ...
    private _valClose(item){
        
        let miExito=true
        if (item['alCerrar']!=undefined) {
            miExito=item['alCerrar']() 
            if (typeof miExito != "boolean") miExito=true
        }
        return miExito
    }
    ...
    protected clearCont(cont, main=false):any{
        const lista:any[]=Array.from(cont.querySelectorAll("*"))
        ...
        for(let i=0; i<lista.length; i++){
            ...
            if (verHijos == undefined) return {error:""}
            if (verHijos['error']!=""){
                return verHijos
            }
        } 
