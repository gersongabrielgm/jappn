...
export default class MetaSelect extends JappN{
    ...
    alCargar(props){
        if (props.local != undefined) this.rest.local=props.local
        else this.rest.local=this.local
        ...
        return <div class={"_trj MetaSelect "+ (props.class==undefined?"":props.class) } id={props.id}>
    ...
    buscar(){
        ...
        const tag = dlg.alCargar({
            mensaje:<div style="height:calc(100% - 15px)">
                <Input etiqueta="Criterio para búsqueda" onKeyUp={(e)=>this.filtrar(e)}/>
                <div class="resultado" style="overflow:auto;height:calc(100% - 110px)"></div>
            </div>,
            //botones:{Buscar:{}},
            bin:this,
    ...
    filtrar(e){
        ...
        const criterio = {}
        if (this.props['criterio'] != undefined) criterio[this.props.criterio] = e.target.value.trim()
        ...
        .then((data:any)=>{
            ...
            if (data.length==0) res.innerHTML = "No hay resultado."
                ...
                res.append(...data.map(i=><div class="item" onClick={()=>this.seleccionar(i[this.llave], btn)}>
    ...
    nuevo(){
        ...
        const form = this.props['form'] != undefined 
            ? this.props['form'] 
            : <Input id="valor" class="_campo" placeHolder="Valor para el nuevo registro." etiqueta={this.props['etiqueta']==undefined?"Nuevo registro":this.props['etiqueta']} />
