class F$ {
    $(consulta) {
        const els = typeof consulta == 'string' ? document.querySelectorAll(consulta) : [consulta];
        if (els.length == 0)
            return false;
        ...
        els.__proto__.append = function (valor, callBack) {
        ...
        els.__proto__.find = function (el) {
            if (typeof el == 'string')
        ...
        els.__proto__.getPadre = function (query) {
            ...
            do {
                if (prefijo == ".")
                    pass = found.classList.contains(valor);
                else if (prefijo == "#")
                    pass = found.id == valor;
                else
                    pass = found.tagName == valor.toUpperCase();
                if (!pass)
        ...
        els.__proto__.val = function (valor = undefined, callBack) {
            if (valor == undefined) {
                ...
                if (this[0].tagName == 'INPUT' || this[0].tagName == 'SELECT' || this[0].tagName == "TEXTAREA")
                    ...
                    if (typeof valor == "string" || typeof valor == "number")
                        this[0].innerText = valor;
                    else
                        this[0].innerHTML = valor;
                }
            }
            return this[0];
        };
        ...
        els.__proto__.addClass = function (clase) { return this.class(clase, 1); };
        els.__proto__.removeClass = function (clase) { return this.class(clase, 0); };
        els.__proto__.class = function (clase, tipo) {
        ...
        els.__proto__.preAddClass = function (clase, tipo) {
            const cls = clase.split(" ");
            for (let i = 0; i < this.length; i++) {
        ...
        els.__proto__.hasClass = function (clase) {
            let retorno = false;
        ...
        els.__proto__.toggleClass = function (clase) {
            const cls = clase.split(" ");
            this.forEach((item) => {
                cls.forEach((cl) => {
                    if (this.$(item).hasClass(cl))
                        this.$(item).removeClass(cl);
                    else
                        this.$(item).addClass(cl);
                });
            });
        };
        els.__proto__.css = function (attr, valor = undefined) {
            this.forEach((item) => {
                if (valor != undefined)
                    item.style[attr] = valor;
            });
        };
        els.__proto__.print = function (info = {}) {
            const el = this[0];
            ...
            if (titulo.innerText != '')
                page.appendChild(page.appendChild(titulo));
            if (cuerpo != undefined)
                page.appendChild(cuerpo);
            if (pie.innerText != '')
                page.appendChild(page.appendChild(pie));
            const p = window.open('');
            if (p !== null) {
        ...
        els.__proto__.hijos = function (query) {
            const hijos = this[0].childNodes;
            if (query == "*")
                return hijos;
            const retorno = [];
