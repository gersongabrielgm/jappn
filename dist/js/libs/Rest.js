...
export default class Rest {
    ...
    superReq = false;
    constructor(cfg = {}) {
        if (cfg['superReq'] != undefined)
            this.superReq = cfg['superReq'];
        ...
        if (this.superReq == true) {
            ...
            const data = localStorage.getItem("cfg_" + cfg['restNamespace']);
            //console.log(data)
            cfg = data != undefined
                ? JSON.parse(data)
                : {};
    ...
    async patch(endPoint, body) {
        if (this.local == true || this.local == 1)
            return await this.localServer.run("PATCH", endPoint, body);
    ...
    async post(endPoint, body) {
        if (this.local == true || this.local == 1)
            return await this.localServer.run("POST", endPoint, body);
        return await fetch(this.restServer + endPoint, {
            method: "POST",
            headers: {
                isrest: "true",
    ...
    async put(endPoint, body) {
        if (this.local == true || this.local == 1)
            return await this.localServer.run("PUT", endPoint, body);
        return await fetch(this.restServer + endPoint, {
            method: "PUT",
            headers: {
                isRest: "true",
    ...
    async delete(endPoint) {
        if (this.local == true || this.local == 1)
            return await this.localServer.run("DELETE", endPoint);
    }
    ...
    async run(method, endPoint, body = undefined) {
        ...
        return await fetch(this.restServer + endPoint, {
            ...
            .then(data => {
            //console.log(data)
            if (data['utoken'] != undefined)
                localStorage['utk_' + this.restNamespace] = data['utoken'];
            if (data['token'] != undefined)
                localStorage['tk_' + this.restNamespace] = data['token'];
            if (data['appusuario_id'] != undefined)
                localStorage['atk_' + this.restNamespace] = data['appusuario_id'];
            if (pre != null)
