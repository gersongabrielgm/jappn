...
class MiReact {
    ...
    mkDrv(jsxClass, props = {}) {
        ...
        if (drv['_permiso'] != undefined) {
            ...
            if (drv._permiso['rol'] != undefined) {
                if (drv._permiso.rol.indexOf(this.UsRol) > -1)
                    permiso = true;
            }
            if (drv._permiso['usuario'] != undefined) {
                if (drv._permiso.usuario.indexOf(this.Us) > -1)
                    permiso = true;
            }
            if (!permiso) {
    ...
    estoEs = (func) => {
        ...
        if (func.tagName != undefined)
            return func.tagName;
        if (typeof func == 'object' && func instanceof JappN)
            return "jappObj";
        return typeof func;
    };
    jsx = (clase, props = {}, ...hijos) => {
    ...
    createElement(jsx, props, ...children) {
        ...
        let drv; //componente heredado de JappN
        ...
        if (arguments[2] != null) {
            for (let i = 2; i < arguments.length; i++) {
                ...
                if (child instanceof Array) {
                    child.map((x) => tag.append(x));
                }
                else {
                    if (typeof child == 'string')
                        tag.insertAdjacentHTML('beforeend', child);
                    else if (child != undefined)
                        tag.appendChild(child.nodeType == null ? document.createTextNode(child.toString()) : child);
                }
            }
        }
        ...
        if (drv != undefined) {
            if (drv['_preCargado'] != undefined) {
                ...
                if (drv.alCargado != undefined)
                    drv.alCargado(tag, props);
            }
            if (drv['_rendered'] != undefined)
                drv._rendered(tag, props);
    ...
    setPropiedades(element, props) {
        for (let name in props) {
            switch (name) {
                case 'onBlur':
                    element.onblur = (e) => props[name](e);
                    break;
                case 'onKeyDown':
                    element.onkeydown = (e) => props[name](e);
                    break;
                case 'onLoad':
                    props[name](element);
                    break;
                case 'onMouseUp':
                    element.onmouseup = (e) => props[name](e);
                    break;
                case 'onMouseMove':
                    element.onmousemove = (e) => props[name](e);
                    break;
                case 'onMouseOver':
                    element.onmouseover = (e) => props[name](e);
                    break;
                case 'onMouseOut':
                    element.onmouseout = (e) => props[name](e);
                    break;
                case 'onTouchStart':
                    element.ontouchstart = (e) => props[name](e);
                    break;
                case 'onTouchMove':
                    element.ontouchmove = (e) => props[name](e);
                    break;
                case 'onTouchEnd':
                    element.ontouchend = (e) => props[name](e);
                    break;
                case 'onDragEnter':
                    element.ondragenter = (e) => props[name](e);
                    break;
                case 'onDragLeave':
                    element.ondragleave = (e) => props[name](e);
                    break;
                case 'onChange':
                    element.onchange = (e) => props[name](e);
                    break;
                case 'onDrop':
                    element.ondrop = (e) => props[name](e);
                    break;
                case 'onDragEnd':
                    element.ondragend = (e) => props[name](e);
                    break;
                case 'onDragStart':
                    element.ondragstart = (e) => props[name](e);
                    break;
                case 'onDrag':
                    element.ondrag = (e) => props[name](e);
                    break;
                case 'onClick':
                case 'onDClick':
                    ...
                    element.onclick = (e) => {
                        ...
                        if (b.onclick == null && !click)
                            return; //evita el click en objetos que no tienen evento click
                        if (lock)
                            return; //evita el click en objetos que no tienen evento click
                    ...
                    break;
                case 'onKeyUp':
                    element.onkeyup = props[name];
                    break;
                case 'onKeyPress':
                    element.onkeypress = props[name];
                    break;
                default:
