...
class FMenu extends JappN {
    ...
    alCargar(props) {
        ...
        return React.createElement("div", { class: "FMenu" },
            React.createElement("button", { onClick: () => this.visible(), class: "nmo", style: "height:20px;margin:4px" }),
            props.data.map((r, l) => React.createElement("div", null, r.map((i, n) => React.createElement("button", { onClick: () => this.run({ key: "F" + (n + 1) }) },
                "F",
                n + 1,
                React.createElement("span", { click: "true" }, i.nombre))))));
    }
    alCargado(el) {
        this.el = el;
        el['run'] = (e) => {
